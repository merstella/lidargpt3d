...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer):
        ...
        # setting a patch_dropout of 0. would mean it is disabled and this function would be the identity fn
        self.patch_dropout = PatchDropout(self.patch_dropout) if self.patch_dropout > 0. else nn.Identity()
        self.visual = point_transformer


    def forward(self, pts, colors=None, return_xyz=False):
        # divide the point cloud in the same form. This is important
        if pts.shape[-1] > 3:
            if colors is None:
                colors = pts[..., 3:6].contiguous()
            pts = pts[..., :3].contiguous()
        else:
            pts = pts.contiguous()
        
        if colors is None:
            colors = torch.ones_like(pts) * 0.4
        ...
        group_input_tokens = self.encoder(features)          # [B, G, C_enc]
        group_input_tokens = self.encoder2trans(group_input_tokens)  # [B, G, trans_dim]
        ...
        xyz = torch.cat([cls_xyz, center], dim=1)   # [B, G+1, 3]
        x = torch.cat([x, xyz], dim=-1)              # [B, G+1, trans_dim + 3]
        ...
        x = self.trans2embed(x)       # [B, N', embed_dim]

        if return_xyz:
            return x, pc_xyz          # xyz đã được drop đúng token
