...
from minigpt4.models_new.base_model import disabled_train
from minigpt4.models_new.minigpt_base import MiniGPTBase
from minigpt4.models_new.Qformer import BertConfig, BertLMHeadModel
import logging
...
class MiniGPT_3D(MiniGPTBase):
    ...
    def __init__(
            ...
            pc_encoder_backend="uni3d",
            uni3d_ckpt_path=None,
            uni3d_timm_model="eva02_tiny_patch14_224",
            uni3d_drop_path_rate=0.2,
    ):
        super().__init__(
        ...
        self.pc_encoder_backend = pc_encoder_backend
        self.pc_encoder = self.init_pc_encoder(
            pc_precision,
            freeze_pc,
            pc_encoder_backend=pc_encoder_backend,
            uni3d_ckpt_path=uni3d_ckpt_path,
            uni3d_timm_model=uni3d_timm_model,
            uni3d_drop_path_rate=uni3d_drop_path_rate,
        )
        self.pc_feature_dim = getattr(self.pc_encoder, "embed_dim", self.pc_encoder.trans_dim)

        print('Create the MLP: point_2_Qformer_proj')
        if pc_linear_layer == 1:
            projection_layers = []
            projection_layers.append(nn.Linear(self.pc_feature_dim, 1408))
            self.point_2_Qformer_proj = nn.Sequential(*projection_layers)
        elif pc_linear_layer == 2:
            self.point_2_Qformer_proj = nn.Sequential(
                nn.Linear(self.pc_feature_dim, 768),
                nn.GELU(),
                nn.Linear(768, 1408),
            )
        ...
        if self.positional_enc:
            ...
            # learnable angle positional embedding
            # will be broadcast to [B, N, C]
            self.angle_pos_embd = nn.Parameter(
                torch.randn(self.NUM_VIEWS, 1, 1, self.pc_feature_dim)
            )

            self.angle_pos_proj = nn.Sequential(
                nn.Linear(self.pc_feature_dim, self.pc_feature_dim),
                nn.GELU(),
                nn.Linear(self.pc_feature_dim, 1408)
            )
        ...
        if self.use_MQE:
            ...
            self.query_router = nn.Sequential(
                nn.Linear(self.pc_feature_dim * 2, 256),
                nn.GELU(),
    ...
    def compute_point_angles(self, xyz):
        """
        xyz: [B, N, 3]
        return: angles in degree [B, N]
        """
        center = xyz.mean(dim=1, keepdim=True)       # [B,1,3]
        rel = xyz - center                           # [B,N,3]

        angles = torch.atan2(rel[..., 1], rel[..., 0])  # atan2(y, x)
        angles = angles * 180.0 / math.pi
        angles = (angles + 360.0) % 360.0
        return angles

    def build_view_masks(self, angles):
        """
        angles: [B, N]
        return: [6, B, N] boolean masks
        """
        B, N = angles.shape
        masks = torch.zeros(
            (self.NUM_VIEWS, B, N),
        ...
        for i, ranges in enumerate(self.VIEW_RANGE):
            mask = torch.zeros_like(angles, dtype=torch.bool)
            for r in ranges:
                mask |= (angles >= r[0]) & (angles <= r[1])
            masks[i] = mask
        return masks
    
    def get_projected_angle_pos_embd(self, xyz):
        """
        xyz: [B, N+1, 3]  (xyz[0] = CLS)
        return: [B, N+1, D]
        """
        B, N1, _ = xyz.shape
        xyz_points = xyz[:, 1:, :]          # 🔥 bỏ CLS
        N = xyz_points.size(1)

        angles = self.compute_point_angles(xyz_points)    # [B, N]
        view_masks = self.build_view_masks(angles)         # [6, B, N]

        angle_embd = self.angle_pos_embd.expand(
            -1, B, N, -1
        )                                                   # [6, B, N, C]

        angle_embd = (view_masks.unsqueeze(-1) * angle_embd).sum(dim=0)
        angle_embd = self.angle_pos_proj(angle_embd)       # [B, N, D]

        # ✅ CLS angle = 0
        cls_angle = torch.zeros(
            B, 1, angle_embd.size(-1),
            device=angle_embd.device,
            dtype=angle_embd.dtype
        )

        angle_embd = torch.cat([cls_angle, angle_embd], dim=1)
        return angle_embd

    def _run_pc_encoder(self, pc=None, xyz=None, feat=None, return_xyz=False):
        if pc is not None:
            if self.pc_encoder_backend == "uni3d":
                return self.pc_encoder(pc, return_xyz=return_xyz)
            return self.pc_encoder(pts=pc, return_xyz=return_xyz)

        if xyz is not None and feat is not None:
            if self.pc_encoder_backend == "uni3d":
                return self.pc_encoder(xyz, colors=feat, return_xyz=return_xyz)
            return self.pc_encoder(xyz=xyz, feat=feat, return_xyz=return_xyz)
        ...
        if self.use_MQE:
            ...
            if self.router_type == 'Sparse Router':
                with self.maybe_autocast():
                    ...
                    pc_feature, pc_xyz = self._run_pc_encoder(
                        pc=pc, xyz=xyz, feat=feat, return_xyz=True
                    )
                    ...
                    if self.positional_enc:
                        angle_pos = self.get_projected_angle_pos_embd(pc_xyz).to(device)
                        pc_embeds = pc_embeds + angle_pos
                    # MLP
            ...
            with self.maybe_autocast():
                ...
                pc_embeds = self.point_2_Qformer_proj(pc_feature)

                if self.positional_enc:
                    angle_pos = self.get_projected_angle_pos_embd(pc_xyz)
                    pc_embeds = pc_embeds + angle_pos
    ...
    def from_config(cls, cfg):
        ...
        uni3d_drop_path_rate = cfg.get("uni3d_drop_path_rate", 0.2)
