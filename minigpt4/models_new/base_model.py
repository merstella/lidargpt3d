...
class BaseModel(nn.Module):
    ...
    def init_pc_encoder(
        ...
        backend = pc_encoder_backend.lower()

        if backend == "uni3d":
            from .uni3d.point_encoder import PointcloudEncoder
            import timm

            if not uni3d_ckpt_path:
                raise ValueError("`uni3d_ckpt_path` must be provided when `pc_encoder_backend=uni3d`.")

            ckpt_path = os.path.expanduser(uni3d_ckpt_path)
            if not os.path.isabs(ckpt_path):
                ckpt_path = os.path.abspath(ckpt_path)
            if not os.path.isfile(ckpt_path):
                raise FileNotFoundError(f"Uni3D checkpoint not found: {ckpt_path}")

            point_transformer = timm.create_model(
                uni3d_timm_model,
                pretrained=False,
                drop_path_rate=uni3d_drop_path_rate,
            )
            ...
            for key, value in state_dict.items():
                if key.startswith("point_encoder."):
                    pc_state_dict[key.replace("point_encoder.", "")] = value

            point_encoder.load_state_dict(pc_state_dict, strict=False)
            logging.info(f"Using Uni3D point encoder from {ckpt_path}.")

        elif backend == "pointbert":
            ...
            # Preserve legacy packed-point path for Objaverse compatibility.
            ...
            logging.info(f"Using PointBERT point encoder with {point_encoder.point_dims}D points.")
        else:
            raise ValueError(f"Unsupported `pc_encoder_backend`: {pc_encoder_backend}")
